  ...
  async generateReceiptPdf(donationId: string): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const invoiceNo =
          donation.reference || donation._id.toString().substring(0, 12).toUpperCase();
        ...
        for (const item of items) {
          ...
          doc.text(item.name, tableLeft + col1Width + 20, y + 12, {
            width: col2Width - 40,
            align: 'center',
          });
          doc.text(
        ...
        doc.text(
          `${currencySymbol} ${donation.totalAmount.toLocaleString()}`,
          totalBoxX + 15,
          totalBoxY + 32,
        );
