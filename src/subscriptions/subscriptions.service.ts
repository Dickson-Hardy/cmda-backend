...
import { SUBSCRIPTION_PRICES, LIFETIME_MEMBERSHIPS } from './subscription.constant';
import { json2csv } from 'json-2-csv';
import { PricingService } from '../pricing/pricing.service';
import { PaymentFrequency, UserRole as PricingUserRole } from '../pricing/pricing.entity';
import { SubscriptionPaginationQueryDto } from './dto/subscription-pagination.dto';
import { EmailService } from '../email/email.service';
import { UserRole } from '../users/user.constant';
import { PaypalService } from '../paypal/paypal.service';

// Helper function to map User UserRole to Pricing UserRole
const mapUserRoleToPricingRole = (
  userRole: UserRole,
  yearsOfExperience?: string,
): PricingUserRole => {
  switch (userRole) {
    case UserRole.STUDENT:
      return PricingUserRole.STUDENT;
    case UserRole.DOCTOR:
      // Check if doctor is senior based on years of experience
      if (
        yearsOfExperience?.toLowerCase()?.includes('above') ||
        yearsOfExperience?.toLowerCase()?.includes('5 years and above')
      ) {
        return PricingUserRole.DOCTOR_SENIOR;
      }
      return PricingUserRole.DOCTOR;
    case UserRole.GLOBALNETWORK:
      return PricingUserRole.GLOBAL_NETWORK;
    default:
      return PricingUserRole.STUDENT;
  }
};
  ...
  async init(id: string, subscriptionData?: any): Promise<ISuccessResponse> {
    ...
    if (user.role === UserRole.GLOBALNETWORK && subscriptionData) {
      if (subscriptionData.selectedTab === 'lifetime') {
        // Lifetime membership - use pricing service
        amount = await this.pricingService.getLifetimePrice(subscriptionData.lifetimeType);
        isLifetime = true;
        ...
        // Regular subscription with income-based pricing - use pricing service
        const paymentFreq =
          subscriptionData.paymentFrequency === 'monthly'
            ? PaymentFrequency.MONTHLY
            : PaymentFrequency.ANNUAL;
        amount = await this.pricingService.getIncomeBasedPrice(
          subscriptionData.incomeBracket,
          paymentFreq,
        );
      ...
      // Standard pricing for other roles - use pricing service
      const userRoleForPricing = mapUserRoleToPricingRole(user.role, user.yearsOfExperience);

      amount = await this.pricingService.getSubscriptionPrice(
        userRoleForPricing,
        PaymentFrequency.ANNUAL,
      );
    }
